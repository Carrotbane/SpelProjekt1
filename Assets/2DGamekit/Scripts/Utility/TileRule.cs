...
namespace UnityEngine
{
	[Serializable]
	[CreateAssetMenu]
	public class TileRule : TileBase
	{
				...
				if (RuleMatches(rule, position, tileMap, ref transform))
				{
					switch (rule.m_Output)
					{
							...
							case TilingRule.OutputSprite.Random:
								int index = Mathf.Clamp(Mathf.RoundToInt(Mathf.PerlinNoise((position.x + 1000000f) * rule.m_PerlinScale, (position.y + 1000000f) * rule.m_PerlinScale) * rule.m_Sprites.Length), 0, rule.m_Sprites.Length - 1);
		...
		public bool RuleMatches(TilingRule rule, Vector3Int position, ITilemap tilemap, ref Matrix4x4 transform)
		{
			...
			for (int angle = 0; angle <= (rule.m_AutoTransform == TilingRule.AutoTransform.Rotated ? 270 : 0); angle += 90)
			{
			...
			if ((rule.m_AutoTransform == TilingRule.AutoTransform.MirrorX) && RuleMatches(rule, position, tilemap, true, false))
			{
			...
			if ((rule.m_AutoTransform == TilingRule.AutoTransform.MirrorY) && RuleMatches(rule, position, tilemap, false, true))
			{
				transform = Matrix4x4.TRS(Vector3.zero, Quaternion.identity, new Vector3(1f, -1f, 1f));
				return true;
			}
